# NOTE: putting all of this in a module avoid exposing the helper vars/functions to the shell.
# Only `export`ed stuff will be available in the shell when `use`d.
module myprompt {
  ...
  # note: there can only be ONE 'export-env'
  export-env {
    # -- For current prompt
    ...
    # -- For past prompt
    $env.TRANSIENT_PROMPT_COMMAND = { create_old_left_prompt }
    $env.TRANSIENT_PROMPT_COMMAND_RIGHT = { create_old_right_prompt }

    # The prompt indicators represents the right-most part of the left (main) prompt
    # It is used to represent the state of the prompt (normal/insert/multiline/…)
    # -- Indicators for current prompt
    $env.PROMPT_INDICATOR = { " > " }
    $env.PROMPT_INDICATOR_VI_INSERT = {
      let res = color_surround {fg: green3a attr: b} {"I"}
      $" ($res)> "
    }
    $env.PROMPT_INDICATOR_VI_NORMAL = {
      let res = color_surround {fg: blue attr: b} {"N"}
      $" ($res)> "
    }
    $env.PROMPT_MULTILINE_INDICATOR = {||
      let res = color_surround {fg: grey39} {":::"}
      $"($res) "
    }

    # -- Indicators for past prompts
    $env.TRANSIENT_PROMPT_INDICATOR = {
      let res = color_surround {fg: grey46 attr: b} {"%"}
      $" ($res) "
    }
...
# NOTE: only `export`ed stuff is imported in-scope
use myprompt
