...
in the active `coder-*` skills, and ask the user whether to add them.

## When to load

Load this skill when all of the following are true:
- A `coder-*` skill was active during the session.
...
1. **Scan the session** — Review what was written or changed in the script.
   Identify anything that:
    ...
    - Deviates from or extends the active `coder-*` skills.
...
2. **Identify candidate rules** — For each pattern found, decide:
    - Is it general enough to belong in `coder-generic`?
...
- Never add rules that are already covered in `coder-generic` or the active lang skill.
