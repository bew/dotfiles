...
IMPORTANT: Never review the draft inline by reading files and summarizing yourself.
Review MUST run via the `opencode-reviewer` subagent.
If the user asks to "review", "check", or "verify" a draft, load `opencode-crafter`,
enter `Phase:Review`, and invoke the subagent — do not review files manually.
...
Otherwise — invoke the `opencode-reviewer` subagent immediately.
