...
-- When used from `d`elete action, acts as the word + spaces after or before (nicer delete!)
toplevel_map{mode="o", key="w", desc="select whole word", opts={expr=true}, action=function()
  return vim.v.operator == "d" and "aw" or "iw"
end}
toplevel_map{mode="o", key="W", desc="select abc(lkj) whole WORD", opts={expr=true}, action=function()
  return vim.v.operator == "d" and "aW" or "iW"
end}
-- V: Select current word & WORD
toplevel_map{mode="v", key="w", desc="select whole word", action="iw"}
toplevel_map{mode="v", key="W", desc="select whole WORD", action="iW"}
-- NOTE: `iw` (and other `i`/`a` text-objects) have weeiird behavior in visual mode when 2+ chars
--   are selected (or doing `2iw`..), where `iw` doesn't reselect only current word,
--   but instead EXTENDS the visual selection based on which end of the selection the cursor is:
--   - On the left edge, visual selection is extended by word/spaces backward
--   - On the right edge, visual selection is extended by word/spaces forward
--   => It's actually the intended behavior! See `:h v_object-select`
--
--    v
--   f|oo    bar
--    ^
...
--   |---------|  v2iw
--
--   Can be seen also in omap
--
-- For original `yw` / `cw` there is `ye` / `ce`
-- For original `dw` there is no equivalent (delete to next word including spaces)
-- => Restore original `w`/`W` behavior with `Alt-w`/`Alt-W` for v:count support
--    (to re-gain original behavior of `dw`/`dW` using `d<M-w>`/`d<M-W>`)
