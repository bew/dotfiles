...
---@param path string
---@return boolean
function U.fs.path_exists(path)
...
---@param path string
---@return boolean
function U.fs.is_absolute(path)
  if path:match[[\]] then
    -- on windows
    return path:match[[^%a:\]]
  else
...
---@param path string
---@return string
function U.fs.make_path_absolute(path)
...
---@return string
function U.fs.simplify_path(path, relative_to)
  if not relative_to then
    return vim.fn.fnamemodify(path, ":p:~:.")
  end
  -- FIXME: can we simplify this impl?
  local rel_path ---@type string?
  if U.fs.is_subpath(relative_to, path) then
...
---@return boolean
function U.fs.is_subpath(root, candidate)
  if candidate == "" then
    return false
  end
  local root = vim.fs.normalize(U.fs.make_path_absolute(root))
  -- Trim trailing "/" from the root
  if root:find("/", -1) then
    root = root:sub(1, -2)
  end
  candidate = vim.fs.normalize(U.fs.make_path_absolute(candidate))
